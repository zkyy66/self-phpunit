...
//         self::writeStdOut( $fileName,  $str );
    ...
    * Array
    ...
    public static function writeStdOut( $file, $content ) {
        fwrite( STDOUT, 'File:' . $file . ' Content:' . $content );
    }
    ...
    public static function writeStdErr( $file, $content ) {
        fwrite( STDERR, 'File:' . $file . ' Content:' . $content );
    }
     ...
     */
    public static function getFromConfig($str)
    {
        if (!$str) return '';
    
        $ret = Yaf_Registry::get('config')->get($str);
        if (!$ret) {
            $ret = Yaf_Registry::get('appConfig')->get($str);
        }
        if (is_string($ret)) {
            return $ret;
        }
